...
public class GhostMove2 : MonoBehaviour
{
    ...
    public GameGUINavigation2 GUINav;
    public PlayerController2 pacman;
    private GameManager2 _gm;
    ...
    void Start()
    {
        _gm = GameObject.Find("Game Manager2").GetComponent<GameManager2>();
    ...
    void FixedUpdate()
    {
        ...
        if (GameManager2.gameState == GameManager2.GameState.Game)
        {
            animate();
...
10 8";
    ...
    void OnTriggerEnter2D(Collider2D other)// metodo de colisao que verifica se é perdida uma vida caso os ghosts nao estiverem no estado run, ou se estiverem, aumenta o score e destroy os ghosts
    {
        if (other.name == "pacman")
        {
            if (state == State.Run)
            {
                ...
                var SCORE = GameObject.FindGameObjectWithTag("ScoreText").GetComponent<ScoreCounter2>();
    ...
    void RunAway()
    {
        ...
        else GetComponent<AI2>().RunLogic();// inicia a lógica de fugir
    }
