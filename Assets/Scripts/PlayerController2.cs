...
public class PlayerController2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GN = GameObject.Find("UI Manager").GetComponent<GameGUINavigation2>();
        _dest = transform.position;
    }

    void FixedUpdate()
    {
        switch (GameManager2.gameState)
        {
            case GameManager2.GameState.Game:
                ReadInputAndMove();
                Animate();
                break;

            case GameManager2.GameState.Dead:
                if (!_deadPlaying)
    ...
    IEnumerator PlayDeadAnimation()
    {
        ...
        if (pacman.GetComponent<PlayerController2>().lives <= 0)// caso o user perca a ultima vida
        {
