...
public class GhostMove3 : MonoBehaviour
{
    ...
    public GameGUINavigation3 GUINav;
    public PlayerController3 pacman;
    private GameManager3 _gm;
    ...
    void Start()
    {
        _gm = GameObject.Find("Game Manager3").GetComponent<GameManager3>();
    ...
    void FixedUpdate()
    {
        ...
        if (GameManager3.gameState == GameManager3.GameState.Game)
        {
            animate();
...
10 8";
    ...
    void OnTriggerEnter2D(Collider2D other)// metodo de colisao que verifica se é perdida uma vida caso os ghosts nao estiverem no estado run, ou se estiverem, aumenta o score e destroy os ghosts
    {
        if (other.name == "pacman")
        {
            if (state == State.Run)
            {
                ...
                var SCORE = GameObject.FindGameObjectWithTag("ScoreText").GetComponent<ScoreCounter3>();
    ...
    void RunAway()
    {
        ...
        else GetComponent<AI3>().RunLogic();// inicia a lógica de fugir
    }
